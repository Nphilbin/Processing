...
void setup() {
  size(320, 240);
  background(255);
  PImage img =loadImage("color_wheel_730.png");
}
...
void draw() {
  ...
  image(img, wheelInsert, wheelInsert);
  noStroke();
  fill(selectedColor);
...
boolean mouseInColorWheel() {
  return (mouseX < wheelInsert + widthColorWheel && mouseY < wheelInsert + heightColorWheel);
}
